          ...
          @endif
			...
			<div class="panel panel-inverse mt-4">
				...
				<div class="panel-heading bg-blue-700 text-white">
                    <h4 class="panel-title">Data Pengajuan Surat Kematian</h4>
					...
					<table id="data-table-default" class="table table-striped table-bordered align-middle">
						<thead>
							<tr>
								...
								<th class="text-nowrap">Nama Pelapor</th>
								<th class="text-nowrap">Alamat Pelapor</th>
								<th class="text-nowrap">Nama Jenazah</th>
                                <th class="text-nowrap">Alamat Jenazah</th>
						...
						<tbody>
							@foreach ($data_kematian as $kematian)
          						<tr>
									...
									<td>{{$kematian->alamat_pelapor}}</td>
									<td>{{$kematian->nama_jenazah}}</td>
									<td>{{$kematian->alamat_jenazah}}</td>
									<td>{{$kematian->status}}</td>
									<td class="text-center">
										<a href="/kematian/{{$kematian->id}}/edit" class="btn btn-primary btn-circle"><i class="fas fa-info-circle"></i></a>
										@if(auth()->user()->role=='Administrator')
										<a href="/kematian/{{$kematian->id}}/cetakPdf" target="blank" class="btn btn-warning btn-circle"><i class="fas fa-download"></i></a>
										<a href="/kematian/{{$kematian->id}}/delete" class="btn btn-danger btn-circle" onclick="return confirm('Yakin menghapus data?')" ><i class="fas fa-trash"></i></a>
										@endif
									</td>
							</tr>
							@endforeach
